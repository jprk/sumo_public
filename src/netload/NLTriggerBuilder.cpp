...
NLTriggerBuilder::parseAndBuildOverheadWireSection(MSNet& net, const SUMOSAXAttributes& attrs) {
    ...
    /*
     * Try to handle both old-style and new style definitions of overhead wires.
     * The old definition references <overheadWireSegment> elements using segments="..." attribute.
     * The new definition that is also generated by netedit references directly lanes of the SUMO network.
     */
    ...
    if (!attrs.hasAttribute(SUMO_ATTR_LANES)) {
        ...
        if (!new_style) {
            throw ProcessError("Definition of overhead wire section: SUMO_ATTR_LANES attribute exists but no SUMO_ATTR_ID is defined.");
        }
        ...
        for (auto lit = laneIDs.begin(); lit != laneIDs.end(); lit++)
        {
            ...
            std::string segmentID = "owsgmnt_" + *lit;
            // Store it to the list of segments that shall be considered later
            ...
            if (lane == nullptr) {
                ...
                if ((*lit)[0] != ':' || MSGlobals::gUsingInternalLanes) {
                    // Throw the exception only in case that the lane really does not exist in the network file
                    // or it is broken.
                    throw InvalidArgument("The lane '" + *lit + "' referenced by <overheadWire> element '" + id + "' does not exist or is broken.");
                }
            }
            if (lane == nullptr || lane->isInternal()) {
            ...
            if (myHandler->checkStopPos(frompos, topos, lane->getLength(), POSITION_EPS, friendlyPos) != SUMORouteHandler::StopPos::STOPPOS_VALID) {
                frompos = 0.0;
                topos = lane->getLength();
                WRITE_MESSAGE("The overhead wire segment '" + segmentID + "' has wrong stop position. Automatically set from 0 to the length of the lane '" + *lit + "'.");
            }
        ...
        if (!voltageSources.empty())
        {
            ...
            throw InvalidArgument("The <overheadWire> element '" + id + "' does not contain lane(s) " + s + " where the substation shall be connected.");
        }
    }

    /*
     * Add overhead wire segments over internal lanes
     * This is kept separate from the above code as it works with overhead wire segments that have been either
     * explicitly created (the "old" definitions) or that were created from the lane list specified in the definition
